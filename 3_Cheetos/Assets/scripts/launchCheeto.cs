...
public class launchCheeto : MonoBehaviour {
	...
	public Vector3 cheetopult1 = new Vector3 (0f, 20f, -9f);
	public float damping = 0.7f; // Friction every frame velocity decreases by a third
	bool fired = false;
	...
	void Update () {
		...
		if ( Time.time > 5 && fired == false ) { //fires only once
		...
		// Manage forces. makes sure ball eventually comes to a stop
